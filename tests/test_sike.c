...
int cryptorestore_kem()
{
    unsigned int i;
    unsigned int LOOPS = (1 << 20);
    unsigned char rsk[CRYPTO_SECRETKEYBYTES] = {0};
    unsigned char rpk[CRYPTO_PUBLICKEYBYTES] = {0};
    unsigned char tsk[CRYPTO_SECRETKEYBYTES] = {0};
    unsigned char tpk[CRYPTO_PUBLICKEYBYTES] = {0};
    unsigned long long cycles_keygen = 0, cycles1, cycles2;

    printf("\n\nBENCHMARKING ISOGENY-BASED KEY RESTORE MECHANISM %s\n", SCHEME_NAME);
    printf("--------------------------------------------------------------------------------------------------------\n\n");
    
    crypto_kem_keypair(rpk, rsk);

    for (i = 0; i < LOOPS; i++)
    {
        cycles1 = cpucycles();
        crypto_kem_restorekey(tpk, tsk, rpk);
        cycles2 = cpucycles();
        cycles_keygen = cycles_keygen + (cycles2 - cycles1);
    }

    printf("  Key generation runs in average ............................... %10lld ", cycles_keygen/LOOPS); print_unit;
    printf("\n\n");
